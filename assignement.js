...
// Or
for (const book of books) {
  if (typeof book.author === "string") allAuthors.push(bookAuthor);
  else {
...
// Check if bookMap has the author key. and if so, log "" to the console.

bookMap.get("author") ? console.log("The author of the book is known") : "";
...
const normalizeAuthorName = (authorName) => {
  const nameNormalized = authorName.toLowerCase().trim();
  const name = nameNormalized.endsWith("(contributor)")
    ? nameNormalized.slice(0, nameNormalized.indexOf("(contributor)"))
    : "";
...
// -----------------------------
// Working with Strings - Part 3 split || join || padEnd||PadStart||repeat
// -----------------------------
...
const logBookCategories = (categories) => {
  ...
  for (const cat of categoriesArr) console.log(cat);
};
